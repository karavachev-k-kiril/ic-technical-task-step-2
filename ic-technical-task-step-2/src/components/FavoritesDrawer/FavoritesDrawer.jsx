...
const FavoritesDrawer = ({ favorites, onToggleFav }) => {
  ...
  return (
    <div
      className={`favorites-drawer ${isOpen ? 'open' : 'closed'} ${!favorites.length ? 'empty' : ''}`}
    >
      <div className="drawer-header" onClick={toggleDrawer}>
        <h3 className="drawer-title">Your Favorites ({favorites.length})</h3>
        <div className="drawer-controls">
          ...
          >
            {isOpen ? (
              ...
              favorites.length > 0 && <ChevronDownIcon />
            )}
          </button>
        </div>
      </div>
        ...
        <div className="drawer-content">
          {favorites.length > 0 ? (
            ...
            <div className="no-favorites-container-drawer">
              ...
              <p className="no-favorites-text-drawer">
                Click the heart icon on any workshop to save it here.
              </p>
            </div>
          )}
        </div>
