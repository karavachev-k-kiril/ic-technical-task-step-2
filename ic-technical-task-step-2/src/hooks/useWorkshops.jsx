...
export const useWorkshops = (initialCountry, initialPage, initialPerPage) => {
  ...
  const performFetch = useCallback(
    async (params, appendResults = false) => {
      setError(null);
      const {
        country,
        pageToFetch,
      ...
      try {
        ...
        setPagination(
          data.pagination || { page: 1, perPage: 10, totalPages: 1, total: 0 }
        );
        ...
        setError(
          `Failed to fetch workshop data: ${err.message}. The API might be down or has strict limits.`
        );
        if (!appendResults) {
          setWorkshops([]);
  ...
  useEffect(() => {
    ...
    if (perPage === 'All') {
      ...
      performFetch(fetchParams, false).finally(() => setIsLoading(false));
    } else {
      setInfiniteScrollPage(1);
      setHasMore(false);
      performFetch(fetchParams, false).finally(() => setIsLoading(false));
  ...
  const loadMoreWorkshops = useCallback(async () => {
    ...
    try {
      ...
      const fetchParams = {
        ...
        perPageToFetch: MAX_API_PER_PAGE,
        availability,
        brand,
        query,
      };
